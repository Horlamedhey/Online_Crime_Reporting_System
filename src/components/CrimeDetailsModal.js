...
import { CheckIcon, StarIcon, ChevronDownIcon } from "@chakra-ui/icons";
import { badge as statuses } from "@/data";
...
export default function CrimeDetailsModal({ isOpen, closeModal, currentCase }) {
  ...
  const possibleStatus = Object.entries(statuses)
    .filter(([key]) => key != "all")
    .map(([key]) => key);

  const upgradeStatus = async () => {
    setUpdatingStatus(true);
    const index = possibleStatus.findIndex((e) => e == status);
    setStatus(possibleStatus[index + 1]);
    currentCase.status = possibleStatus[index + 1];
    const { error } = await supabase
      .from("crimes")
      .update({ status: possibleStatus[index + 1] })
            ...
            <ModalBody>
              <Box>
                ...
                <Flex justify="space-between" align="center">
                  <Badge
                    variant="subtle"
                    colorScheme={statuses[status].color}
                    p={2}
                    borderRadius="lg"
                    my="4px"
                  >
                ...
                <Flex justify="space-between" mt={2} gap={2}>
                  <Box w="65%">
                    ...
                    <Box mt="20px">
                      ...
                      <GridItem display="flex" align="center" my="8px">
